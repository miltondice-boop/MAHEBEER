...
function addControls(){
 const grid=document.getElementById('weeklyGrid');if(!grid||!employeeCache.length)return;
 ...
 cards.forEach(card=>{
  ...
  const emp=employeeCache.find(e=>text.includes(String(e.nombre_completo||'').trim().toLowerCase()));
  if(!emp)return;
...
async function init(){css();try{await getEmployees();addControls();const grid=document.getElementById('weeklyGrid');if(grid)new MutationObserver(addControls).observe(grid,{childList:true,subtree:true})}catch(e){console.error('Ajustes de personal:',e)}}
